...
module.exports = function(S) {
  ...
  class RuntimeStreamline extends S.classes.Runtime {
    ...
    getName(providerName) {
      if (providerName === 'aws') {
        ...
        return RuntimeStreamline.getName();
      }
    }
    ...
    run(func, stage, region) {
      ...
      return BbPromise.all([
        ...
        .spread((event, envVars) => {
          const childArgs = [__dirname + '/streamline-runner'];
    ...
    build(func, stage, region) {
      ...
      return this.createDistDir(func.name).then(pathDist => {
        return this.copyFunction(func, pathDist, stage, region)
          ...
          .then(() => this._streamline(func, pathDist))
    ...
    getBabelOptions(func) {
      return _.defaults(_.get(func, 'custom.runtime.streamline'), {
        presets: ['es2015']
      });
    }

    _streamline(func, pathDist) {

      SCli.log('\n_streamline ----------------- func: ' + func);
      console.log(func);
      SCli.log('getRootPath: ' + func.getRootPath());


      const srcFilename = path.resolve(func.getRootPath(), 'handler.js');

      SCli.log('--------- pathDist: ' + pathDist + '\n');
      SCli.log('--------- srcFilename: ' + srcFilename + '\n');

      //const streamlined = streamline.transformFileSync(srcFilename, {});

      //console.log('\n\nstreamlined:');
      //console.log(streamlined.code);
      //console.log('\n\n');
      ...
      SCli.log('handlerFileName ----------------- ' + handlerFileName);
      ...
      // MAKE BOGUS "bundle()" METHOD FOR NOW
      let b = {
        bundle: function(cb){

          streamline.compile(function(err, result){
            if(err){
              console.error('ERROR AFTER BUNDLE', err);
            }
            else{
              console.log('BUNDLE SUCCESS!', result);
            }
            cb(err, result);
          }, [func.getRootPath()], {outDir: pathDist});



          //console.log('BOGUS .bundle() method!!');
          //cb(null);
          //return Promise.resolved('SOME KIND OF RESOLUTION');
        },
        writeToFileAsync: function(pathBundle, buf){
          //writeToFileAsync
          console.log('BOGUS writeToFileAsync');
        }
      };
      ...
      SCli.log('pathBundle ----------------- ' + pathBundle);

      return BbPromise.fromCallback(cb => b.bundle(cb))
          .then((buf) => b.writeToFileAsync(pathBundle, buf));
